...
<td width="310" align="left" ><a href="forumss_new-topico.jsp"><img src="images/novotopico.gif" width="112" height="24" border="0" /></a></td>
...
<table  width="580" height="61" border="1" cellpadding="0" cellspacing="0" background="#EAEADB">
  ...
  <tr>
	...
	$rd .= 	"<form method='post' action='forumss-comments-".$r['Msg_Id']."-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='forumss-comments-".$r['Msg_Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
	
	} elseif ($GLOBALS['user_auth']->username == $r['NickName']) {
	$rd .= 	"<form method='post' action='forumss-comments-".$r['Id']."-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='forumss-comments-".$r['Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
              ...
              <table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
                <tr>
                  <td  style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><? 
						if ($data =='') {
							echo '<form method="post" action="forumss-comments-'.$_GET['do'].'-new.jsp">';
							echo' 
							<br>
						...
						<table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
							...
							echo '<form method="post" action="forumss-comments-'.$_GET['do'].'-edit.jsp">';
				...
				if ($db->Affected_Rows() > 0) {
					...
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `forumss2` (`Title`,`Comment`, `Author`, `NickName`, `CountryGrade`, `signforum`, `Date`,`Msg_Id`) VALUES (?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
													array($_POST['title'],$_POST['elm1'],$user['Id'],$game['NickName'],$game['CountryGrade'],$user['signforum'],$_GET['do']));
									
							$db->Execute("Update forumss set lastpost = ? where Id=?", array(substr(trim($db->DBTimeStamp(time())), 1,19),$_GET['do']));													
										if ($db->Affected_Rows()== 1) {
													echo notice('Comment has been added successfully...<br> Redirecting in 5 seconds!','Success!!');
													header('Refresh: 2; url=forumss-comments-'.$_GET['do'].'.jsp');
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
				...
				foreach ($result->GetArray() as $rs => $r) {
						 echo "
							   ...
							   <td>".$r['Id']."</td>
							    ...
							    <td align=\"left\" > 
								<form method='post' action='forumss-admin-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='forumss-admin-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>
			...
			case 'new':
				...
				$db->Execute('Insert into `forumss` (`Title`,`Text`,`Text2`,`Author`,`Date`,`Comments`) VALUES (?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=forumss-admin.jsp');
				}
			break;
			case 'delete':
