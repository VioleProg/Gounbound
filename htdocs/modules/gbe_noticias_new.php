...
$db->Execute("CREATE TABLE IF NOT EXISTS `gbe_noticias` (
...
$db->Execute("CREATE TABLE IF NOT EXISTS `gbe_comentarios` (
`Id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`Msg_Id` INT NOT NULL ,
...
<tr>
 ...
 <td width="15" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left">&nbsp;</td>
    ...
    <td width="200" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left"><a href="gbe_noticias-comments-'.$r['Id'].'.jsp#comments"> '.$r['Title'].'</a> </td>
		  <td width="60" align="center" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2">&nbsp;<a href="gbe_noticias-comments-'.$r['Id'].'.jsp#comments">'.$r['Comments'].'</a></td>
...
global $config;
 $rd = '<table width="494" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	$rd .= 	"<form method='post' action='gbe_noticias_new-comments-".$r['Msg_Id']."-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='gbe_noticias_new-comments-".$r['Msg_Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
	
	} elseif ($GLOBALS['user_auth']->username == $r['Author']) {
	$rd .= 	"<form method='post' action='gbe_noticias_new-comments-".$r['Msg_Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from gbe_noticias2 where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
...
<table width=100%><tr><td width="66">&nbsp;</td><td>
						<? 
						if ($data =='') {
							echo '<form method="post" action="gbe_noticias_new-comments-'.$_GET['do'].'-new.jsp">';
							echo '<h3>Create New Comment</h3>'; 
							echo '<label>Title </label><input type="text"  name="title" value="RE: '.$title.'" size="40" maxlength="45">';
							} else {
							echo '<form method="post" action="gbe_noticias_new-comments-'.$_GET['do'].'-edit.jsp">';
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from gbe_noticias where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
			...
			<!-- /TinyMCE -->
						...
						<? 
						if ($data =='') {
							echo '<form method="post" action="gbe_noticias_new-topico-new.jsp">';
							...
							echo '<form method="post" action="gbe_noticias_new-topico-edit.jsp">';
...
switch (@$_GET['url']) {
	...
	$result = $db->Execute("Select * from gbe_noticias order by Date desc");
	...
	if (isset($_GET['do'])) {
				...
				$result = $db->Execute("Select * from gbe_noticias where Id=? order by Date desc", array($_GET['do']));
				if ($db->Affected_Rows() > 0) {
					foreach ($result->GetArray() as $rs => $r) {
						echo news_template($r);
						$title = $r['Title'];
					}
					$result = $db->Execute("Select * from gbe_noticias2 where Msg_Id=? order by Date desc", array($_GET['do']));
					if ($db->Affected_Rows() > 0) {
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `gbe_comentarios` (`Title`,`Comment`,`Author`,`Date`,`Msg_Id`) VALUES (?,?,?,'.$db->DBTimeStamp(time()).',?)',
										...
										if ($db->Affected_Rows()== 1) {
													...
													header('Refresh: 2; url=gbe_noticias_new-comments-'.$_GET['do'].'.jsp');
													$db->Execute("Update gbe_noticias set Comments = Comments + 1 where Id = ?", array($_GET['do']));
										}
							break;
							...
							if ($config['user_login']=='ok') {
								$result = $db->Execute("Delete from gbe_noticias2 where Id=? ", array($_POST['Id']));
									if ($db->Affected_Rows() > 0) {
									...
									$result = $db->Execute("Delete from gbe_noticias2 where Id=? and Author=?", array($_POST['Id'],$user['NickName']));
										if ($db->Affected_Rows() > 0) {
											echo notice('Comment has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
											header('Refresh: 2; url=gbe_noticias_new-comments-'.$_GET['do'].'.jsp');
											$db->Execute("Update gbe_noticias set Comments = Comments - 1 where Id = ?", array($_GET['do']));
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
			...
			case 'new':
				...
				$db->Execute('Insert into `gbe_noticias` (`Title`,`Text`,`Text2`,`Author`,`NickName`,`CountryGrade`,`signforum`,`class`,`Date`,`Comments`) VALUES (?,?,?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=forum4.jsp');
				}
			break;
			case 'delete':
				$db->Execute('delete from `gbe_noticias` where Id=?',	array($_POST['Id']));
				if ($db->Affected_Rows()== 1) {
							echo notice('Announcement has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
							header('Refresh: 2; url=gbe_noticias_new-topico.jsp');
				}
			break;
			...
			$db->Execute('update `gbe_noticias` set Title=?, `Text`=?, `Text2` = ?, Author = ? where Id=?',	array($_POST['title'],$_POST['elm1'],$_POST['elm2'],$user['NickName'],$_POST['id']));
				if ($db->Affected_Rows()== 1) {
