...
$data = $_GET['url'];
   SmartyPaginate::connect($data);
	...
	SmartyPaginate::setUrl("index2.php?op=avatarshop3", $data);
...
if (!isset($_GET['url'])) {
	...
	$where = '';
	if (isset($_POST['avatarshop3'])) {
		if (clean_variable($_POST['avatarshop3'],1) == true) {
		echo notice('Variable Error Found... Try Again :p','Error');
		} else {
		$where = $_POST['avatarshop3'];
		}
	} 
	...
	if (clean_variable($_GET['url'],1)==true) {
	  ...
	  header('Refresh: 2; url=avatarshop3.jsp');
	} else {
		...
		$result = $db->Execute('Select *, shop_avatar.id as Nick, shop_avatar.id from shop_avatar where shop_avatar.id=shop_avatar.id and shop_avatar.nome = ? order by nome ',array($_GET['url']));
