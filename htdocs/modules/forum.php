...
$data = 'forum-player';
	...
	SmartyPaginate::setUrl("index.php?op=forum", $data);
	//SmartyPaginate::setUrl("rank-player.asp", $data);

		  function get_db_results($data,$where='',$order='') {
     global $db, $config;
		$where = $where == '' ? 'forumlivre.NickName = forumlivre.NickName' : "forumlivre.Id = forumlivre.Id and forumlivre.Id like '%$where%'";
        $result = $db->Execute("SELECT SQL_CALC_FOUND_ROWS *, forumlivre.Id, forumlivre.Id FROM forumlivre where $where order by forumlivre.Date DESC LIMIT ?,?",
...
';
if ($r['class'] = 'linkforumnormal') {
$classe = '';
}
elseif ($r['class'] = 'linkforumred') {
$classe = 'Guild: ';
}
elseif ($r['class'] = 'linkforumblue') {
$classe = 'Fixo: ';
}
elseif ($r['class'] = 'linkforumorange') {
$classe = 'Anúncio: ';
}
	if (is_array($rs)) {
		foreach ($rs as $db => $r) {
...
<td width="207" align="left" class="'.$r['class'].'">&nbsp;&nbsp;&nbsp;<a href="forumlivre-comments-'.$r['Id'].'.jsp">'.$classe.$r['Title'].'</a></td>
