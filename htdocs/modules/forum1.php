	...
	?>
      ...
      <table width="100%" border="0" class="ranksch">
        <tr >
          ...
          <td width="567" align="right" ><a href="forumss_new-topico.jsp"><img src="template/gbetools/images/novotopico.gif" width="112" height="24" border="0" /></a></td>
        </tr>
      </table>
      <?PHP 
}
...
$data = 'forum-player';
	...
	SmartyPaginate::setUrl("index.php?op=forum1", $data);
	//SmartyPaginate::setUrl("rank-player.asp", $data);

		  function get_db_results($data,$where='',$order='') {
     global $db, $config;
		$where = $where == '' ? 'forumss.NickName = forumss.NickName' : "forumss.Id = forumss.Id and forumss.Id like '%$where%'";
        $result = $db->Execute("SELECT SQL_CALC_FOUND_ROWS *, forumss.Id, forumss.Id FROM forumss where $where order by forumss.Date DESC LIMIT ?,?",
...
<td width="207" align="left" class="'.$r['class'].'">&nbsp;&nbsp;&nbsp;<a href="forumss-comments-'.$r['Id'].'.jsp">'.$r['Title'].'</a></td>
