	...
	?>
      <table border="0" cellpadding="0" cellspacing="0" width="39%">
        <!-- fwtable fwsrc="topbtforum.png" fwbase="topbtforum.jpg" fwstyle="Dreamweaver" fwdocid = "742308039" fwnested="0" -->
		  ...
		  <?PHP
		  // /*
$db->Execute("CREATE TABLE IF NOT EXISTS `forumss` (
...
$db->Execute("CREATE TABLE IF NOT EXISTS `forumss2` (
`Id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`Msg_Id` INT NOT NULL ,
...
<tr>
 ...
 <td width="15" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left">&nbsp;</td>
    ...
    <td width="200" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left"><a href="forumss-comments-'.$r['Id'].'.jsp#comments"> '.$r['Title'].'</a> </td>
		  <td width="60" align="center" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2">&nbsp;<a href="forumss-comments-'.$r['Id'].'.jsp#comments">'.$r['Comments'].'</a></td>
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from forumss2 where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
...
<table width=100%><tr><td width="66">&nbsp;</td><td>
						<? 
						if ($data =='') {
							echo '<form method="post" action="forumss_new-comments-'.$_GET['do'].'-new.jsp">';
							echo '<h3>Create New Comment</h3>'; 
							echo '<label>Title </label><input type="text"  name="title" value="RE: '.$title.'" size="40" maxlength="45">';
							} else {
							echo '<form method="post" action="forumss_new-comments-'.$_GET['do'].'-edit.jsp">';
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from forumss where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
			...
			<!-- /TinyMCE -->
						...
						<? 
						if ($data =='') {
							echo '<form method="post" action="forumss_new-topico-new.jsp">';
							echo '<table width="100%" border="0" align="center">
                              <tr>
                                <td align="center"><img src="template/gbevolution/images/novotopico.jpg" width="662" height="60" /></td>
                              </tr>
                            </table>
                            <table width="100%" border="0" align="center" bgcolor="#F7F7F7" class="list02">
                              <tr>
							...
							echo '<form method="post" action="forumss_new-topico-edit.jsp">';
					...
					Additional Content: ( if page cut is done this content will be hidden.. )<br>
						<textarea id="elm1" name="elm2" rows="15" cols="80" width="80%" height="50%">
...
switch (@$_GET['url']) {
	...
	$result = $db->Execute("Select * from forumss order by Date desc");
	...
	if (isset($_GET['do'])) {
				...
				$result = $db->Execute("Select * from forumss where Id=? order by Date desc", array($_GET['do']));
				if ($db->Affected_Rows() > 0) {
					foreach ($result->GetArray() as $rs => $r) {
						echo news_template($r);
						$title = $r['Title'];
					}
					$result = $db->Execute("Select * from forumss2 where Msg_Id=? order by Date desc", array($_GET['do']));
					if ($db->Affected_Rows() > 0) {
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `forumss2` (`Title`,`Comment`,`Author`,`Date`,`Msg_Id`) VALUES (?,?,?,'.$db->DBTimeStamp(time()).',?)',
										...
										if ($db->Affected_Rows()== 1) {
													...
													header('Refresh: 2; url=forumss_new-comments-'.$_GET['do'].'.jsp');
													$db->Execute("Update forumss set Comments = Comments + 1 where Id = ?", array($_GET['do']));
										}
							break;
							...
							if ($config['user_login']=='ok') {
								$result = $db->Execute("Delete from forumss2 where Id=? ", array($_POST['Id']));
									if ($db->Affected_Rows() > 0) {
									...
									$result = $db->Execute("Delete from forumss2 where Id=? and Author=?", array($_POST['Id'],$user['NickName']));
										if ($db->Affected_Rows() > 0) {
											echo notice('Comment has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
											header('Refresh: 2; url=forumss_new-comments-'.$_GET['do'].'.jsp');
											$db->Execute("Update forumss set Comments = Comments - 1 where Id = ?", array($_GET['do']));
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
			...
			case 'new':
				...
				$db->Execute('Insert into `forumss` (`Title`,`Text`,`Text2`,`Author`,`NickName`,`CountryGrade`,`signforum`,`class`,`Date`,`Comments`) VALUES (?,?,?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=forum1.jsp');
				}
			break;
			case 'delete':
				$db->Execute('delete from `forumss` where Id=?',	array($_POST['Id']));
				if ($db->Affected_Rows()== 1) {
							echo notice('Announcement has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
							header('Refresh: 2; url=forumss_new-topico.jsp');
				}
			break;
			...
			$db->Execute('update `forumss` set Title=?, `Text`=?, `Text2` = ?, Author = ? where Id=?',	array($_POST['title'],$_POST['elm1'],$_POST['elm2'],$user['NickName'],$_POST['id']));
				if ($db->Affected_Rows()== 1) {
