...
if (@$config['admin_login'] == 'ok') {
	...
	?><td width="310" align="left" ><a href="fevento_new-topico.jsp"><img src="images/novotopico.gif" width="112" height="24" border="0" /></a></td>
<?PHP 
} elseif (@$config['user_login'] == 'ok') { echo '<td width="310" align="left" ></td>';
}
	?>
...
<div class="maintitle_3" style="margin-top:10px;"><div class="left"><div class="right"><div class="main_text">'.$r['Title'].' </div></div></div></div>
  ...
  <tr>
    <td width="170" align="center" valign="top" class="row2" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><table width="170" border="0" align="center">
      ...
      <tr>
        <td align="left"><img src="images/aff_tick.gif" width="12" height="12" /> <a href="perfil-'.$r['NickName'].'.sys">Ver Perfil </a></td>
      </tr>
      <tr>
        <td align="left"><img src="images/aff_tick.gif" width="12" height="12" /> <a href="topicomsg-'.$r['NickName'].'.sys">Posts do(a) <span class="row2" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;">'.$r['NickName'].'</span></a></td>
      </tr>
      <tr>
...
<table  width="580" height="61" border="1" cellpadding="0" cellspacing="0" background="#EAEADB">
  ...
  <tr>
	...
	if ($config['admin_login'] == 'ok') {
	$rd .= 	"<form method='post' action='fevento-comments-".$r['Msg_Id']."-edit.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='fevento-comments-".$r['Msg_Id']."-delete.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
	
	} elseif ($GLOBALS['user_auth']->username == $r['NickName']) {
	$rd .= 	"<form method='post' action='fevento-comments-".$r['Id']."-edit.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='fevento-comments-".$r['Id']."-delete.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
              ...
              <table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
                <tr>
                  <td  style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><? 
						if ($data =='') {
							echo '<form method="post" action="fevento-comments-'.$_GET['do'].'-new.sys">';
							echo' 
							<br>
						...
						<table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
							...
							echo '<form method="post" action="fevento-comments-'.$_GET['do'].'-edit.sys">';
...
switch (@$_GET['url']) {
	...
	if (isset($_GET['do'])) {
				...
				if ($db->Affected_Rows() > 0) {
					...
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `fevento2` (`Title`,`Comment`, `Author`, `NickName`, `CountryGrade`, `signforum`, `Date`,`Msg_Id`) VALUES (?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
													array($_POST['title'],$_POST['elm1'],$user['Id'],$game['NickName'],$game['CountryGrade'],$user['signforum'],$_GET['do']));
									
							$db->Execute("Update fevento set lastpost = ? where Id=?", array(substr(trim($db->DBTimeStamp(time())), 1,19),$_GET['do']));													
										if ($db->Affected_Rows()== 1) {
													echo notice('Comment has been added successfully...<br> Redirecting in 5 seconds!','Success!!');
													header('Refresh: 2; url=fevento-comments-'.$_GET['do'].'.sys');
				...
				header('Refresh: 2; url=index.sys');
			}
	break;
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
				...
				foreach ($result->GetArray() as $rs => $r) {
						 echo "
							   ...
							   <td>".$r['Id']."</td>
							    ...
							    <td align=\"left\" > 
								<form method='post' action='fevento-admin-edit.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='fevento-admin-delete.sys'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>
			...
			case 'new':
				...
				$db->Execute('Insert into `fevento` (`Title`,`Text`,`Text2`,`Author`,`Date`,`Comments`) VALUES (?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=fevento-admin.sys');
				}
			break;
			case 'delete':
		...
		header('Refresh: 2; url=entrar.sys');
