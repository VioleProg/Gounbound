...
$data = 'avatar';
	...
	SmartyPaginate::setUrl("index2.php?op=avatarshop", $data);
	//SmartyPaginate::setUrl("avatar.jsp", $data);

    function get_db_results($data,$where='',$order='') {
     global $db, $config;
		$where = $where == '' ? 'shop_avatar.id = shop_avatar.id' : "nome = nome and tipo like '%$where%'";
        $result = $db->Execute("SELECT SQL_CALC_FOUND_ROWS *, shop_avatar.id, shop_avatar.id FROM shop_avatar where $where order by shop_avatar.id LIMIT ?,?",
	...
	$where = '';
	if (isset($_POST['nome'])) {
		if (clean_variable($_POST['nome'],1) == true) {
