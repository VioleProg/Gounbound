...
<table width="100%" border="0" class="ranksch">
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="39%">
        <!-- fwtable fwsrc="topbtforum.png" fwbase="topbtforum.jpg" fwstyle="Dreamweaver" fwdocid = "742308039" fwnested="0" -->
          ...
          <td background="template/gbetools/images/topbtforum_r1_c3.jpg" width="26"><a href="forum2.jsp" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('forumdenuncias','','template/gbetools/images/topbtforum_r1_c6.jpg',1)"><img src="template/gbetools/images/topbtforum_r1_c6.jpg" alt="F&oacute;rum denuncias" name="forumdenuncias" border="0" id="forumdenuncias" /></a></td>
          <td background="template/gbetools/images/topbtforum_r1_c3.jpg" width="26"><a href="forum3.jsp" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('forumeventos','','template/gbetools/images/topbtforum_r1_c8.jpg',1)"><img src="template/gbetools/images/topbtforum_r1_c08.jpg" alt="F&oacute;rum denuncias" name="forumeventos" border="0" id="forumeventos" /></a></td>
...
$db->Execute("CREATE TABLE IF NOT EXISTS `fevento` (
...
$db->Execute("CREATE TABLE IF NOT EXISTS `fevento2` (
`Id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`Msg_Id` INT NOT NULL ,
...
<tr>
  <td width="26" align="left" class="row2" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><span class="row2" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><img src="template/gbevolution/images/newtopics.gif" width="22" height="21" /></span></td>
 <td width="15" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left">&nbsp;</td>
 
    <td width="200" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2" align="left"><a href="fevento-comments-'.$r['Id'].'.jsp#comments"> '.$r['Title'].'</a> </td>
		  <td width="60" align="center" style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;" class="row2">&nbsp;<a href="fevento-comments-'.$r['Id'].'.jsp#comments">'.$r['Comments'].'</a></td>
...
global $config;
 $rd = '<table width="494" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>&nbsp;</td>
    <td ><img src="template/'.$config['template'].'/template/gbevolution/images/comment_lin.gif" width="428" height="16"></td>
  </tr>
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from fevento2 where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
...
<table width=100%><tr><td width="66">&nbsp;</td><td>
						<? 
						if ($data =='') {
							echo '<form method="post" action="fevento_new-comments-'.$_GET['do'].'-new.jsp">';
							echo '<h3>Create New Comment</h3>'; 
							echo '<label>Title </label><input type="text"  name="title" value="RE: '.$title.'" size="40" maxlength="45">';
							} else {
							echo '<form method="post" action="fevento_new-comments-'.$_GET['do'].'-edit.jsp">';
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from fevento where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
			...
			<!-- /TinyMCE -->
						...
						<? 
						if ($data =='') {
							echo '<form method="post" action="fevento_new-topico-new.jsp">';
							echo '<table width="662" border="0" align="center">
                              <tr>
                                <td align="center"><img src="template/gbevolution/images/novotopico.jpg" width="662" height="60" /></td>
                              </tr>
                            </table>
                            <table width="659" border="0" align="center" bgcolor="#F7F7F7" class="list02">
                              <tr>
                                ...
                                <td width="571" bgcolor="#FFFFFF"><input type="text"  name="title" size="50" maxlength="30" class="textfield" /></td>
                              </tr>
                              <tr>
							...
							echo '<form method="post" action="fevento_new-topico-edit.jsp">';
...
switch (@$_GET['url']) {
	...
	$result = $db->Execute("Select * from fevento order by Date desc");
	...
	if (isset($_GET['do'])) {
				...
				$result = $db->Execute("Select * from fevento where Id=? order by Date desc", array($_GET['do']));
				if ($db->Affected_Rows() > 0) {
					foreach ($result->GetArray() as $rs => $r) {
						echo news_template($r);
						$title = $r['Title'];
					}
					$result = $db->Execute("Select * from fevento2 where Msg_Id=? order by Date desc", array($_GET['do']));
					if ($db->Affected_Rows() > 0) {
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `fevento2` (`Title`,`Comment`,`Author`,`Date`,`Msg_Id`) VALUES (?,?,?,'.$db->DBTimeStamp(time()).',?)',
										...
										if ($db->Affected_Rows()== 1) {
													...
													header('Refresh: 2; url=fevento_new-comments-'.$_GET['do'].'.jsp');
													$db->Execute("Update fevento set Comments = Comments + 1 where Id = ?", array($_GET['do']));
										}
							break;
							...
							if ($config['user_login']=='ok') {
								$result = $db->Execute("Delete from fevento2 where Id=? ", array($_POST['Id']));
									if ($db->Affected_Rows() > 0) {
									...
									$result = $db->Execute("Delete from fevento2 where Id=? and Author=?", array($_POST['Id'],$user['NickName']));
										if ($db->Affected_Rows() > 0) {
											echo notice('Comment has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
											header('Refresh: 2; url=fevento_new-comments-'.$_GET['do'].'.jsp');
											$db->Execute("Update fevento set Comments = Comments - 1 where Id = ?", array($_GET['do']));
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
			...
			case 'new':
				...
				$db->Execute('Insert into `fevento` (`Title`,`Text`,`Text2`,`Author`,`NickName`,`CountryGrade`,`signforum`,`class`,`Date`,`Comments`) VALUES (?,?,?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=forum3.jsp');
				}
			break;
			case 'delete':
				$db->Execute('delete from `fevento` where Id=?',	array($_POST['Id']));
				if ($db->Affected_Rows()== 1) {
							echo notice('Announcement has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
							header('Refresh: 2; url=fevento_new-topico.jsp');
				}
			break;
			...
			$db->Execute('update `fevento` set Title=?, `Text`=?, `Text2` = ?, Author = ? where Id=?',	array($_POST['title'],$_POST['elm1'],$_POST['elm2'],$user['NickName'],$_POST['id']));
				if ($db->Affected_Rows()== 1) {
