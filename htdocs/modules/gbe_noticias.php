...
<table  width="580" height="61" border="1" cellpadding="0" cellspacing="0" background="#EAEADB">
  ...
  <tr>
	...
	if ($config['admin_login'] == 'ok') {
	$rd .= 	"<form method='post' action='gbe_noticias-comments-".$r['Msg_Id']."-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='gbe_noticias-comments-".$r['Msg_Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
	
	} elseif ($GLOBALS['user_auth']->username == $r['NickName']) {
	$rd .= 	"<form method='post' action='gbe_noticias-comments-".$r['Id']."-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='gbe_noticias-comments-".$r['Id']."-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>";
...
if ($data != '') {
global $db;
$result = $db->Execute("Select * from gbe_comentarios where Id=?",array($data));
$rs = $result->GetArray();
$r = $rs[0];
}
			?>
              ...
              <table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
                <tr>
                  <td  style="background:#EAEADB;border-top:1px solid #F5F5E7;border-left:1px solid #F5F5E7;"><? 
						if ($data =='') {
							echo '<form method="post" action="gbe_noticias-comments-'.$_GET['do'].'-new.jsp">';
							echo' 
							<br>
						...
						<table width="580" height="35" border="1" align="center" cellpadding="0" cellspacing="0" background="#EAEADB" class="borderwrap" nowrap="nowrap">
							...
							echo '<form method="post" action="gbe_noticias-comments-'.$_GET['do'].'-edit.jsp">';
...
$r = $rs[0];
			...
			?>
              ...
              <? 
						if ($data =='') {
							echo '<form method="post" action="gbe_noticias-admin-new.jsp">';
							echo '<h3>Create New Announcement</h3>'; 
							echo '<label>Title </label><input type="text"  name="title" size="30" maxlength="30">';
							} else {
							echo '<form method="post" action="gbe_noticias-admin-edit.jsp">';
...
switch (@$_GET['url']) {
	...
	if (isset($_GET['do'])) {
				...
				if ($db->Affected_Rows() > 0) {
					...
					$result = $db->Execute("Select * from gbe_comentarios where Msg_Id=? order by Date desc", array($_GET['do']));
					if ($db->Affected_Rows() > 0) {
					echo '<a name="comments">';
						...
						if ($config['user_login'] == 'ok') {
							switch (@$_GET['key']) {
							case 'new':
									$db->Execute('Insert into `gbe_comentarios` (`Title`,`Comment`, `Author`, `NickName`, `CountryGrade`, `signforum`, `Date`,`Msg_Id`) VALUES (?,?,?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
													array($_POST['title'],$_POST['elm1'],$user['Id'],$game['NickName'],$game['CountryGrade'],$user['signforum'],$_GET['do']));
									
							$db->Execute("Update gbe_noticias set lastpost = ? where Id=?", array(substr(trim($db->DBTimeStamp(time())), 1,19),$_GET['do']));													
										if ($db->Affected_Rows()== 1) {
													echo notice('Comment has been added successfully...<br> Redirecting in 5 seconds!','Success!!');
													header('Refresh: 2; url=gbe_noticias-comments-'.$_GET['do'].'.jsp');
							...
							if ($config['user_login']=='ok') {
								$result = $db->Execute("Delete from gbe_comentarios where Id=? ", array($_POST['Id']));
									if ($db->Affected_Rows() > 0) {
									...
									$result = $db->Execute("Delete from gbe_comentarios where Id=? and Author=?", array($_POST['Id'],$user['NickName']));
										if ($db->Affected_Rows() > 0) {
											echo notice('Comment has been Deleted successfully...<br> Redirecting in 5 seconds!','Success!!');
											header('Refresh: 2; url=gbe_noticias-comments-'.$_GET['do'].'.jsp');
											$db->Execute("Update gbe_noticias set Comments = Comments - 1 where Id = ?", array($_GET['do']));
	...
	if ($config['user_login'] == 'ok') {
		...
		switch(@$_GET['do']) {
				...
				foreach ($result->GetArray() as $rs => $r) {
						 echo "
							   ...
							   <td>".$r['Id']."</td>
							    ...
							    <td align=\"left\" > 
								<form method='post' action='gbe_noticias-admin-edit.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='Edit'>
							</form>
								<form method='post' action='gbe_noticias-admin-delete.jsp'>
							<input type='hidden' name='Id' value='".$r['Id']."' class='buttons' >
							<input type=submit class='buttons' name=submit value='DEL'>
							</form>
			...
			case 'new':
				...
				$db->Execute('Insert into `gbe_noticias` (`Title`,`Text`,`Text2`,`Author`,`Date`,`Comments`) VALUES (?,?,?,?,'.$db->DBTimeStamp(time()).',?)',
				...
				if ($db->Affected_Rows()== 1) {
							...
							header('Refresh: 2; url=gbe_noticias-admin.jsp');
				}
			break;
			case 'delete':
