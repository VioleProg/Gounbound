...
{ 
  ...
  for ($i=0; $i<count($rules); $i++)
  {
    ...
    switch ($requirement)
    {
	 ...
	 case "capt":
		if( $_SESSION['security_code'] == $fields[$field_name] && !empty($_SESSION['security_code'] ) ) {
      ...
      case "existe_login":       
        if (isset($fields[$field_name]) && jaExisteLogin($fields[$field_name]))
          $errors[] = $error_message;
        break;

      case "nao_existe_login":       
        if (isset($fields[$field_name]) && !jaExisteLogin($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "existe_nick":       
        if (isset($fields[$field_name]) && jaExisteNick($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "ja_tem_item":       
        if (isset($fields[$field_name]) && jaTemItem($fields[$field_name2],$fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "nao_existe_nick":       
        if (isset($fields[$field_name]) && !jaExisteNick($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "existe_clan":       
        if (isset($fields[$field_name]) && jaExisteClan($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "tem_clan":       
        if (isset($fields[$field_name]) && temClan($fields[$field_name]))
          $errors[] = $error_message;
        break;
	  ...
	  case "existe_email":       
        if (isset($fields[$field_name]) && jaExisteEmail($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "link_ativo":       
        if (isset($fields[$field_name]) && linkAtivo($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "existe_item_shop":       
        if (isset($fields[$field_name]) && !existeItemShop($fields[$field_name]))
          $errors[] = $error_message;
        break;

       case "existe_item_shop2":       
        if (isset($fields[$field_name]) && !existeItemShop2($fields[$field_name]))
          $errors[] = $error_message;
        break;
      
	  case "existe_item_shop3":       
        if (isset($fields[$field_name]) && !existeItemShop3($fields[$field_name]))
          $errors[] = $error_message;
        break;
	  
	  case "existe_item_shop_ponto":       
        if (isset($fields[$field_name]) && !existeItemShopPonto($fields[$field_name]))
          $errors[] = $error_message;
        break;
	  
	  case "existe_item_shop_evento":       
        if (isset($fields[$field_name]) && !existeItemShopEvento($fields[$field_name]))
          $errors[] = $error_message;
        break;

	  case "not_existe_email":       
        if (isset($fields[$field_name]) && !jaExisteEmail($fields[$field_name]))
          $errors[] = $error_message;
        break;
	  ...
	  case "temxcash":
        if (getCash($fields[$field_name]) < $fields[$field_name2])
          $errors[] = $error_message;
        break;
		
		case "temxponto":
        if (getPonto($fields[$field_name]) < $fields[$field_name2])
          $errors[] = $error_message;
        break;

		case "temxcoin":
        if (getCoin($fields[$field_name]) < $fields[$field_name2])
          $errors[] = $error_message;
        break;

	  case "temxgold":
        if (getGold($fields[$field_name]) < $fields[$field_name2])
          $errors[] = $error_message;
        break;
